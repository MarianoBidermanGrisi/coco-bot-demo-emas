...
class TradingBot:
    def __init__(self, config):
        ...
        if not self.trader.check_connection():
            print("❌ No se pudo conectar a BingX. El bot no operará.")
            self.trader = None
    ...
    # === MÉTODOS DE DETECCIÓN (SIN CAMBIOS) ===
...
📍 {expectativa}
    ...
    # === GESTIÓN DE POSICIONES - ADAPTADO PARA BINGX ===
    def calcular_tamaño_posicion(self, symbol, precio_entrada):
        ...
        try:
            ...
            # BingX devuelve una lista de balances
            balance_disponible = 0
            for asset in info_cuenta:
                if asset['asset'] == 'USDT':
                    balance_disponible = float(asset['availableBalance'])
                    break
            
            if balance_disponible <= 0:
                logger_bingx.error("❌ Balance disponible es 0 o negativo")
                return None
            
            monto_usdt = balance_disponible * 0.03  # 3% del balance
            ...
            # Aplicar precisión de cantidad
            quantity_precision = self.trader.get_quantity_precision(symbol)
            cantidad_ajustada = round(cantidad_base, quantity_precision)
            
            # Verificar cantidad mínima
            if 'minQty' in symbol_info and cantidad_ajustada < float(symbol_info['minQty']):
                logger_bingx.warning(f"⚠️ Cantidad {cantidad_ajustada} menor que minQty {symbol_info['minQty']}")
                cantidad_ajustada = float(symbol_info['minQty'])
            
            # Verificación final
            if cantidad_ajustada <= 0:
                logger_bingx.error(f"❌ Cantidad ajustada es 0 o negativa para {symbol}")
                return None
            
            logger_bingx.info(f"✅ Tamaño posición calculado: {cantidad_ajustada} {symbol}")
            return cantidad_ajustada
            
        except Exception as e:
            logger_bingx.error(f"❌ Error calculando tamaño posición para {symbol}: {e}")
            return None
                ...
                if positions and len(positions) > 0:
                    for position in positions:
                        if float(position['positionAmt']) != 0:
                            print(f"   ⏭️  {symbol} tiene posición abierta en BingX")
                            return True
            except Exception as e:
                print(f"⚠️ Error verificando posiciones BingX para {symbol}: {e}")
    ...
    def ejecutar_operacion_bingx(self, simbolo, tipo_operacion, precio_entrada, sl, tp):
        ...
        try:
            ...
            precio_actual = self.trader.get_current_price(simbolo)
            if not precio_actual:
                print(f"❌ No se pudo obtener precio actual para {simbolo}")
                return False
            ...
            # Colocar órdenes de cierre
            sl_order = self.trader.place_stop_loss_order(simbolo, side, sl_ajustado)
            tp_order = self.trader.place_take_profit_order(simbolo, side, tp_ajustado)

            # Verificar estado final de las órdenes
            if not sl_order or not tp_order:
                logger_bingx.warning(f"⚠️ Al menos una orden de cierre falló en {simbolo}. Verificando estado...")
                
                # Verificar órdenes existentes
                ordenes_activas = self.trader.get_open_orders(simbolo)
                ...
                if not sl_existe or not tp_existe:
                    logger_bingx.error(f"❌ Operación en {simbolo} NO protegida adecuadamente")
                    # Intentar recolocar órdenes
                    self.trader.recolocar_ordenes_cierre(simbolo, side, sl_ajustado, tp_ajustado)

            return True
            
        except Exception as e:
            logger_bingx.error(f"❌ Error crítico ejecutando operación en {simbolo}: {e}")
            return False
    ...
    def monitorear_ordenes_activas(self):
        ...
        for simbolo, operacion in list(self.operaciones_activas.items()):
            try:
                ...
                if not ordenes_ok:
                    logger_bingx.error(f"🚨 CRÍTICO: No se pudieron mantener órdenes de cierre en {simbolo}")
    ...
    def verificar_cierre_operaciones(self):
        ...
        try:
            posiciones = self.trader.get_open_positions()
            posiciones_activas = {p['symbol'].replace('-USDT', 'USDT'): float(p['positionAmt']) for p in posiciones if float(p['positionAmt']) != 0}
        except Exception as e:
            print(f"⚠️ Error obteniendo posiciones reales de BingX: {e}")
            posiciones_activas = {}

        for simbolo, operacion in list(self.operaciones_activas.items()):
            # Verificar si la posición sigue abierta en BingX
            posicion_actual = posiciones_activas.get(simbolo, 0.0)
            
            if posicion_actual == 0.0:
                # La posición se cerró en BingX → procesar cierre
                ...
                if not datos_mercado:
                    # Si no podemos obtener datos, usar precio actual de BingX
                    precio_salida = self.trader.get_current_price(simbolo) if self.trader else operacion['precio_entrada']
                else:
                ...
                print(f"     📊 {simbolo} Cierre detectado (posición cerrada en BingX) - PnL: {pnl_percent:.2f}%")

        return operaciones_cerradas

    # === MÉTODOS DE ESTRATEGIA PRINCIPAL (SIN CAMBIOS) ===
    def escanear_mercado(self):
        ...
        for simbolo in self.config.get('symbols', []):
            try:
                # Verificar no operar si ya hay posición activa
                ...
                if self.ejecutar_operacion_bingx(simbolo, tipo_operacion, precio_entrada, sl, tp):
                    ...
                    print(f"❌ Operación en {simbolo} no ejecutada en BingX")
                del self.esperando_reentry[simbolo]
            except Exception as e:
...
🤖 <b>Operación ejecutada en BingX con 5x apalancamiento</b>
🚨 <b>MODO: DINERO REAL</b>
        """
        token = self.config.get('telegram_token')
        chat_ids = self.config.get('telegram_chat_ids', [])
        if token and chat_ids:
...
🏦 Exchange: BingX (Real)
    ...
    def generar_mensaje_cierre(self, datos_operacion):
        ...
        mensaje = f"""
{emoji} <b>OPERACIÓN CERRADA - {datos_operacion['symbol']} (BINGX)</b>
