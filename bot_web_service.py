# bot_web_service.py
# Adaptación para Render del bot Breakout + Reentry + RSI
import requests
import time
import json
import os
...
class TradingBot:
    ...
    def generar_senal_operacion(self, simbolo, tipo_operacion, precio_entrada, tp, sl,
        ...
        rsi_condicion = f"📈 RSI: {info_canal['rsi']:.1f} {'(≥50 → válido para LONG)' if tipo_operacion == 'LONG' else '(<50 → válido para SHORT)'}"
        breakout_texto = ""
        if breakout_info:
...
📊 <b>RSI:</b> {rsi_condicion}
⏰ <b>Hora:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
💡 <b>Estrategia:</b> BREAKOUT + REENTRY + CONFIRMACIÓN RSI y STOCH
        """
    ...
    def generar_grafico_breakout(self, simbolo, info_canal, df, tipo_breakout, config_optima):
        try:
            ...
            fig, axes = mpf.plot(sub_df, type='candle', style='nightclouds',
                                title=f'{simbolo} | {titulo_extra} | {config_optima["timeframe"]} | ⏳ ESPERANDO REENTRY + RSI',
                                ylabel='Precio',
                                addplot=apds,
                                volume=False,
                                returnfig=True,
...
📍 {expectativa}
    ...
    def escanear_mercado(self):
        ...
        print(f"\n🔍 Escaneando {len(simbolos_a_escanear)} de {total} símbolos (Estrategia: Breakout + Reentry + RSI)...")
...
📊 RSI: {datos_operacion.get('rsi', 50.0):.1f}
...
@app.route('/')
def index():
    return "Bot Breakout + Reentry + RSI está en línea.", 200
